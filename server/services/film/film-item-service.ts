/**
 * Film Item Service - TypeScript Migration
 * 
 * Handles film inventory management: purchases, status tracking, roll linking.
 */
...
export const VALID_STATUSES = ['in_stock', 'loaded', 'shot', 'sent_to_lab', 'developed', 'archived'] as const;
export type FilmItemStatus = typeof VALID_STATUSES[number];
...
export interface FilmItem {
  ...
  purchase_shipping_share: number | null;
  purchase_note: string | null;
  develop_lab: string | null;
  develop_process: string | null;
  develop_price: number | null;
  develop_shipping: number | null;
  develop_date: string | null;
  develop_channel: string | null;
...
export interface ListFilmItemsFilters {
...
export interface FilmItemPatch {
  status?: FilmItemStatus;
  label?: string | null;
  purchase_channel?: string | null;
  purchase_vendor?: string | null;
...
export interface LinkFilmItemParams {
  filmItemId: number;
  rollId: number;
  loadedCamera?: string;
  targetStatus?: FilmItemStatus;
}

function assertStatus(status: string): asserts status is FilmItemStatus {
  if (!VALID_STATUSES.includes(status as FilmItemStatus)) {
    throw new Error(`Invalid film item status: ${status}`);
  }
}

export async function createFilmItemsFromPurchase(batch: PurchaseBatch): Promise<Array<{ id: number; film_id: number }>> {
  const {
    purchase_date,
    purchase_channel,
    purchase_vendor,
  ...
  const createdItems: Array<{ id: number; film_id: number }> = [];
...
export async function listFilmItems(filters: ListFilmItemsFilters = {}): Promise<FilmItem[]> {
  const {
    status,
    film_id,
  ...
  return rows as unknown as FilmItem[];
}

export async function getFilmItemById(id: number): Promise<FilmItem | null> {
  const row = await PreparedStmt.getAsync('film_items.getById', [id]);
  return (row as FilmItem) || null;
}

export async function updateFilmItem(id: number, patch: FilmItemPatch): Promise<void> {
  const allowed: Array<keyof FilmItemPatch> = [
    'status', 'label',
    'purchase_channel', 'purchase_vendor', 'purchase_order_id',
    'purchase_price', 'purchase_currency', 'purchase_date',
  ...
  for (const key of allowed) {
    if (Object.prototype.hasOwnProperty.call(patch, key)) {
      if (key === 'status' && patch[key]) assertStatus(patch[key] as string);
      sets.push(`${key} = ?`);
...
export async function softDeleteFilmItem(id: number): Promise<void> {
...
export async function hardDeleteFilmItem(id: number): Promise<void> {
  const item = await getFilmItemById(id);
  if (!item) throw new Error('Film item not found');
  if (item.roll_id) throw new Error('Cannot delete film item that is linked to a roll');
...
export async function linkFilmItemToRoll({
  filmItemId,
  rollId,
  loadedCamera,
  targetStatus = 'shot'
}: LinkFilmItemParams): Promise<{ filmItemId: number; rollId: number }> {
  ...
  const validSourceStatuses: FilmItemStatus[] = ['in_stock', 'loaded', 'shot', 'sent_to_lab'];
