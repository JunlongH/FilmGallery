/**
 * Schema Migration Script - TypeScript Migration
 * 
 * Handles:
 * 1. Creating core tables (films, rolls, photos, tags, etc.)
 * 2. Ensuring columns exist with proper defaults
 * 3. Data fixes and consistency migrations
 * 
...
export async function runSchemaMigration(): Promise<void> {
  ...
  const run = (sql: string, params: unknown[] = []): Promise<RunResult | null> => new Promise((res) => {
    db.run(sql, params, function(err) {
      if (err) res({ error: err }); // Resolve with error to avoid crashing, just log it
      else res(null);
    });
  });
  ...
  try {
    ...
    interface PhotoRow { id: number; roll_id: number }
    const photos = await all<PhotoRow>("SELECT id, roll_id FROM photos WHERE display_seq = 0 OR display_seq IS NULL ORDER BY roll_id, filename");
    if (photos.length > 0) {
      let currentRoll = -1;
      let seq = 0;
    ...
    interface RollRow { id: number; camera?: string; lens?: string; photographer?: string }
    const rolls = await all<RollRow>("SELECT id, camera, lens, photographer FROM rolls");
    log(`Found ${rolls.length} rolls to check.`);
    
    interface GearRow { roll_id: number; type: string; value: string }
    const gearRows = await all<GearRow>("SELECT roll_id, type, value FROM roll_gear");
    const existingGear = new Set<string>();
