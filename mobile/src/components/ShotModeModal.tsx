...
interface CalculatedExposure {
  isValid: boolean;
  ev?: number;
  targetShutter?: string;
  targetAperture?: number;
  requiresFlash?: boolean;
  flashMode?: string;
}

interface ProgramAutoOptions {
  maxAperture: number;
  flashMode: string;
}

type CameraMode = 'av' | 'tv' | 'ps';
type MeteringMode = 'average' | 'spot';
type LocationStatus = 'idle' | 'loading' | 'optimizing' | 'done' | 'failed';
type PickerType = 'aperture' | 'shutter' | 'max_aperture' | null;
...
interface ShotModeModalProps {
  ...
  onUse: (data: {
    ...
    exposureDetails?: CalculatedExposure;
    cameraMode: CameraMode;
    flashMode?: string;
  }) => void;
  filmIso?: number;
  forcePsMode?: boolean;
  forcedMaxAperture?: number | null;
  preloadedLocation?: LocationData | null;
}
...
const PickerModal: React.FC<PickerModalProps> = ({ visible, onClose, data, onSelect, title }) => {
import locationService from '../services/locationService.native';
...
const ShotModeModal: React.FC<ShotModeModalProps> = ({ visible, onClose, onUse, filmIso = 400, forcePsMode = false, forcedMaxAperture = null, preloadedLocation = null }) => {
  ...
  const [iso, setIso] = useState<number>(filmIso || 400);
  ...
  const bumpIso = (direction: number): void => {
    if (!iso) return;
  ...
  const focalToZoom = (focal: number): number => focal / 24;
  ...
  // Exposure modes
  const [selectedAperture, setSelectedAperture] = useState<number>(5.6);
  const [selectedShutter, setSelectedShutter] = useState<string>('1/125');
  ...
  const [liveExposure, setLiveExposure] = useState<ExposureData | null>(null);
  const [diagnosticInfo, setDiagnosticInfo] = useState<DiagnosticInfo>({ frames: 0, fpRunning: false, brightness: 0, ev: 0 });
  
  // Camera mode state
  const [cameraMode, setCameraMode] = useState<CameraMode>(forcePsMode ? 'ps' : 'av'); // av | tv | ps
  const [psFlashMode, setPsFlashMode] = useState<string>('off');
  const [psMaxAperture, setPsMaxAperture] = useState<number>(forcedMaxAperture || 2.8);
  ...
  const handleExposureUpdate = useCallback((data: ExposureData) => {
    if (data?.fpActive) {
  ...
  const measuredSceneEV: number | null = liveExposure?.ev ?? exposureData?.ev ?? diagnosticInfo.ev ?? null;
  ...
  const calculatedExposure: CalculatedExposure = React.useMemo(() => {
  ...
  const activeEV: number | undefined = measuredEV ?? calculatedExposure.ev;

  // Diagnostic logging (reduced frequency - only log once on mount)
  const hasLoggedRef = React.useRef<boolean>(false);
  ...
  const [locationStatus, setLocationStatus] = useState<LocationStatus>('idle'); // 'idle' | 'loading' | 'optimizing' | 'done' | 'failed'
  ...
  const startBackgroundOptimization = async (): Promise<void> => {
    try {
      const watchResult = await locationService.startWatch(
        async (coords: any, accuracy?: number) => {
  ...
  const updateLocationFromCoords = async (coords: any): Promise<void> => {
    try {
  ...
  const handleMeasure = useCallback(async (): Promise<void> => {
  ...
  const handleTapToFocus = useCallback(async (event: any): Promise<void> => {
  ...
  const handleManualSelect = useCallback((value: number | string): void => {
    if (pickerType === 'max_aperture') {
      setPsMaxAperture(value as number);
      return;
    }

    if (pickerType === 'aperture') {
      setSelectedAperture(value as number);
    } else if (pickerType === 'shutter') {
      setSelectedShutter(value as string);
    }
  }, [pickerType]);
