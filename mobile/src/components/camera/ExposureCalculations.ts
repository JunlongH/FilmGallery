...
export const STANDARD_APERTURES: number[] = [
...
export const STANDARD_SHUTTERS: ShutterSpeed[] = [
  { display: '30"', value: 30 },
  { display: '25"', value: 25 },
  { display: '20"', value: 20 },
  { display: '15"', value: 15 },
 ...
 * 将快门字符串解析为秒数
 */
export function parseShutterToSeconds(shutterStr: string | number): number {
 ...
 * 将秒数转换为快门显示字符串
 */
export function secondsToShutterDisplay(seconds: number): string {
  if (seconds >= 1) {
 ...
 * 找到最接近的标准快门
 */
export function findClosestStandardShutter(targetSeconds: number): ShutterSpeed {
 ...
 * 找到最接近的标准光圈
 */
export function findClosestStandardAperture(targetAperture: number): number {
...
export function calculateExposure(
  ...
  let aperture: number, shutter: number, shutterDisplay: string, useFlash = false;
