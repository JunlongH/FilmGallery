...
const SettingsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [backup, setBackup] = useState<string>(backupUrl || '');
  const [isDark, setIsDark] = useState<boolean>(!!darkMode);

  const cleanUrlString = (input: string): string => {
  ...
  const toggleDark = async (val: boolean): Promise<void> => {
    setIsDark(val);
    setDarkMode && setDarkMode(val);
    await AsyncStorage.setItem('theme_dark', val ? 'true' : 'false');
  };

  const testConnection = async (targetUrl: string): Promise<void> => {
