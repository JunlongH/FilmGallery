...
type ViewMode = 'positive' | 'negative';
...
const PhotoViewScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const [snack, setSnack] = useState<SnackState>({ visible: false, msg: '' });
  const [downloading, setDownloading] = useState<boolean>(false);

  const fullUrl = getPhotoUrl(baseUrl, photo, viewMode === 'negative' ? 'negative' : 'full');

  const handleTagsSaved = (newTags: Array<{ id: number; name: string }>): void => {
    setPhoto({ ...photo, tags: newTags });
  };

  const handleNoteSaved = async (newNote: string): Promise<void> => {
    try {
