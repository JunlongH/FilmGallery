...
import { RootStackParamList, ShotLogEntry, Lens } from '../types';

type Props = NativeStackScreenProps<RootStackParamList, 'ShotLog'>;
...
interface FixedLensInfo {
  text: string;
  focal_length?: number;
  max_aperture?: number;
}

interface LensWithDisplay extends Lens {
  displayName: string;
}

function parseShotLog(raw: string | ShotLogEntry[] | null | undefined): ShotLogEntry[] {
  if (!raw) return [];
  try {
    ...
    return data
      .map((entry: Record<string, unknown>) => ({
        ...
        shutter_speed: (entry.shutter_speed as string) || '',
        country: (entry.country as string) || '',
        city: (entry.city as string) || '',
        detail_location: (entry.detail_location as string) || '',
...
const dedupeAndSort = (list: (string | undefined | null)[]): string[] => Array.from(new Set((list || []).filter((x): x is string => Boolean(x)))).sort((a, b) => a.localeCompare(b));

const ShotLogScreen: React.FC<Props> = ({ route, navigation }) => {
  const theme = useTheme();
  const { itemId, filmName, autoOpenShotMode } = route.params || {};
  const [loading, setLoading] = useState<boolean>(true);
  const [saving, setSaving] = useState<boolean>(false);
  const [error, setError] = useState<string>('');
  const [entries, setEntries] = useState<ShotLogEntry[]>([]);
  const [newDate, setNewDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [newShots, setNewShots] = useState<string>('1');
  const [newLens, setNewLens] = useState<string>('');
  const [newAperture, setNewAperture] = useState<string>('');
  const [newShutter, setNewShutter] = useState<string>('');
  const [newCountry, setNewCountry] = useState<string>('');
  const [newCity, setNewCity] = useState<string>('');
  const [newDetail, setNewDetail] = useState<string>('');
  const [lensOptions, setLensOptions] = useState<string[]>(FALLBACK_LENSES);
  const [countries, setCountries] = useState<Country[]>([]);
  const [countryCode, setCountryCode] = useState<string>('');
  const [cities, setCities] = useState<City[]>([]);
  const [showLensOptions, setShowLensOptions] = useState<boolean>(false);
  ...
  const [showShotMode, setShowShotMode] = useState<boolean>(false);
  ...
  const [preloadedLocation, setPreloadedLocation] = useState<PreloadedLocation | null>(null);
  // Fixed lens camera info
  const [fixedLensInfo, setFixedLensInfo] = useState<FixedLensInfo | null>(null);
  const [cameraName, setCameraName] = useState<string>('');
  const [cameraMount, setCameraMount] = useState<string>('');
  // Inventory lenses (native and adapted)
  const [nativeLenses, setNativeLenses] = useState<LensWithDisplay[]>([]); // [{ id, displayName, ... }]
  const [adaptedLenses, setAdaptedLenses] = useState<LensWithDisplay[]>([]); // [{ id, displayName, mount, ... }]

  // Format lens for display
  const formatLensDisplay = (lens: Lens): string => {
