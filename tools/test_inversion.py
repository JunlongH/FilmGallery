...
inverted_lut = invert_3d_lut(gamma_lut, size, verbose=False)
...
# So inverted_lut[0.25][0.25][0.25] should give (0.5, 0.5, 0.5)
test_vals = [0.0, 0.25, 0.5, 0.75, 1.0]
for val in test_vals:
    inv_input = np.array([val, val, val])
    inv_output = apply_inverted(inv_input)
    expected = np.sqrt(val)  # inverse of x^2 is sqrt(x)
    print(f"Inverted LUT input: {val:.2f} -> output: {inv_output[0]:.4f} (expected: {expected:.4f})")
...
for inp in test_inputs:
    ...
    print(f"Input: {inp} -> Gamma: {after_gamma} -> Inverted: {after_inv} | Error: {error:.6f} [{status}]")
