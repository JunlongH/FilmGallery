// src/components/NewRollForm.tsx
...
interface NewRollFormProps {
  onCreated?: () => void;
}

const NewRollForm: React.FC<NewRollFormProps> = ({ onCreated }) => {
  const location = useLocation();
  const [title, setTitle] = useState<string>('');
  const [startDate, setStartDate] = useState<string>('');
  const [endDate, setEndDate] = useState<string>('');
  const [cameraEquipId, setCameraEquipId] = useState<number | null>(null);
  const [lensEquipId, setLensEquipId] = useState<number | null>(null);
  const [selectedCamera, setSelectedCamera] = useState<Camera | null>(null);
  const [photographer, setPhotographer] = useState<string>('');
  const [filmId, setFilmId] = useState<number | null>(null);
  const [exposures, setExposures] = useState<string>(''); //数量
  const [notes, setNotes] = useState<string>('');
  const [isNegative, setIsNegative] = useState<boolean>(false);
  const [files, setFiles] = useState<File[]>([]); // File[]
  const [previews, setPreviews] = useState<{ url: string; tmpName?: string }[]>([]); // { url, tmpName? }
  const [uploadProgress, setUploadProgress] = useState<number | null>(null);
  const [films, setFilms] = useState<Film[]>([]);
  const [useTwoStep, setUseTwoStep] = useState<boolean>(false); // default simplified pipeline
  const [options, setOptions] = useState<MetadataOptions>({ cameras: [], lenses: [], photographers: [] });
  const [dialog, setDialog] = useState<DialogState>({ isOpen: false, title: '', message: '' });
  const [rollLocations, setRollLocations] = useState<LocationData[]>([]);
  const PROCESS_PRESETS = ['C-41', 'E-6', 'BW', 'ECN-2'];
  const [develop, setDevelop] = useState<DevelopData>({ develop_lab: '', develop_process: '', develop_date: '', develop_cost: '', develop_note: '' });
  const [filmItems, setFilmItems] = useState<FilmItem[]>([]);
  const [filmItemId, setFilmItemId] = useState<number | null>(null);
  const [useInventory, setUseInventory] = useState<boolean>(false);
  const [shotLogs, setShotLogs] = useState<ShotLog[]>([]);
  const [fileDates, setFileDates] = useState<Record<string, string>>({}); // { filename: 'YYYY-MM-DD' }
  const [fileMeta, setFileMeta] = useState<Record<string, any>>({});   // { filename: { date, lens } }
  const [applyShotLog, setApplyShotLog] = useState<boolean>(false);
  const [logStartOffset, setLogStartOffset] = useState<number>(0); // Skip first N files when mapping logs
  ...
  useEffect(() => { 
    getFilms().then((f: Film[]) => setFilms(f || [])); 
    getMetadataOptions().then((o: MetadataOptions) => setOptions(o || { cameras: [], lenses: [], photographers: [] }));
    // Fetch all relevant statuses (excluding developed as per request)
    getFilmItems({ status: 'in_stock,loaded,shot,sent_to_lab' }).then((data: any) => {
