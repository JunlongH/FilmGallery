...
const RollGridInner: React.FC<RollGridProps> = ({ rolls = [], horizontal = false }) => {
  const nav = useNavigate();
  const queryClient = useQueryClient();

  function resolveFilmName(roll: Roll | null): string {
    if (!roll) return 'Unknown film';
    const name = roll.film_name_joined || (roll.film && typeof roll.film === 'object' && roll.film.name) || roll.film_type || roll.filmName || (typeof roll.film === 'string' ? roll.film : null) || null;
    return name || 'Unknown film';
  }

  async function onDeleteRoll(id: number, e?: React.MouseEvent): Promise<void> {
    e && e.stopPropagation();
