...
const FilterPanel: React.FC<FilterPanelProps> = ({ isOpen, onChange }) => {
  const [options, setOptions] = useState<MetadataOptions>({ cameras: [], lenses: [], photographers: [], years: [], films: [] });
  ...
  useEffect(() => {
    ...
    getMetadataOptions().then((o: any) => setOptions(prev => ({ ...prev, cameras: o.cameras || [], lenses: o.lenses || [], photographers: o.photographers || [], years: o.years || [] }))).catch(console.error);
    getLocations().then(setLocations).catch(console.error);
    getFilms().then((arr) => setOptions(prev => ({ ...prev, films: Array.isArray(arr) ? arr : [] }))).catch(console.error);
  }, []);

  const toggleValue = (key: keyof FilterState, value: string): void => {
    setFilters(prev => {
  ...
  const toggleYearMonth = (year: string, month: string): void => {
    const ym = `${year}-${month}`;
    setFilters(prev => {
      const list = prev.ym || [];
  ...
  const clearFilters = (): void => {
    const empty: FilterState = { camera: [], lens: [], photographer: [], location_id: [], year: [], ym: [], film: [] };
    setFilters(empty);
    onChange(empty);
    setOpenYears({});
  };

  const normalizeItems = (items: (string | NormalizedItem)[]): NormalizedItem[] => items.map(it => (typeof it === 'object' ? it : { value: String(it), label: String(it) }));
      ...
      {isOpen && (
        ...
        >
          <div style={{ 
            ...
            <Section title="Location" items={locations.map(l=>({ value: String(l.id), label: l.city_name || l.name }))} keyName="location_id" />
