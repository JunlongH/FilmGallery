...
const Favorites: React.FC = () => {
  ...
  const updatePhotoMutation = useMutation({
    mutationFn: ({ photoId, data }: { photoId: number; data: { rating: number } }) => updatePhoto(photoId, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['favorites'] })
  });

  async function onUnlike(photoId: number): Promise<void> {
    try {
...
const FavItem: React.FC<FavItemProps> = ({ p, onSelect, onUnlike }) => {
  ...
  const toggleLike = async (e: React.MouseEvent): Promise<void> => {
    e.stopPropagation();
    onUnlike(p.id);
  };
