...
const SliderControl: React.FC<SliderControlProps> = ({ label, value, onChange, min, max, step=1, onMouseDown, suffix='' }) => {
  const handleMinus = () => {
...
const parseCubeLUT = (text: string): { size: number; data: Float32Array } => {
...
const sampleLUT = (r: number, g: number, b: number, lut: LUT): [number, number, number] => {
  const { size, data } = lut;
  // r, g, b are 0-1
...
export default function FilmInverter({ imageUrl, onClose, onSave }: FilmInverterProps): React.JSX.Element {
  ...
  const [curves, setCurves] = useState<CurvesState>({
    rgb: [...defaultCurve],
    red: [...defaultCurve],
    green: [...defaultCurve],
    blue: [...defaultCurve]
  ...
  // LUTs
  const [lut1, setLut1] = useState<LUT | null>(null);
  const [lut2, setLut2] = useState<LUT | null>(null);
  ...
  const createSpline = (xs: number[], ys: number[]) => {
    const n = xs.length;
    const dys = [], dxs = [], ms = [];
  ...
  const getCurveLUT = (points: CurvePoint[]): Uint8Array => {
  ...
  // Crop Interaction
  const [cropDragAction, setCropDragAction] = useState<CropDragAction | null>(null);
  ...
  const startCropDrag = (e: React.MouseEvent, type: 'move' | 'resize' | 'new', handle: string | null) => {
    e.stopPropagation();
    e.preventDefault();
    if (!canvasRef.current) return;
    const rect = canvasRef.current.getBoundingClientRect();
  ...
  const handleLutUpload = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
