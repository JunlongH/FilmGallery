...
interface Camera {
  ...
  fixed_lens_aperture?: number | null;
  has_builtin_flash?: boolean;
  builtin_flash_id?: number | null;
  fixed_flash_aperture?: number | null;
  [key: string]: any;
}
...
type TabKey = 'cameras' | 'lenses' | 'flashes' | 'films';
...
interface EditFormProps {
  type: TabKey;
  item: EquipmentItem;
  constants: EquipmentConstants | null;
...
const EquipmentManager: React.FC = () => {
  ...
  const [constants, setConstants] = useState<EquipmentConstants | null>(null);
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [editItem, setEditItem] = useState<EquipmentItem | null>(null);
  const [showAddModal, setShowAddModal] = useState<boolean>(false);
  const [confirmDelete, setConfirmDelete] = useState<EquipmentItem | null>(null);
...
const DetailView: React.FC<DetailViewProps> = ({ type, item, onEdit, onDelete, onImageUpload }) => {
  const navigate = useNavigate();
  const [relatedRolls, setRelatedRolls] = useState<any[]>([]);
  const [loadingRolls, setLoadingRolls] = useState<boolean>(false);
...
// Edit Form Component
const EditForm: React.FC<EditFormProps> = ({ type, item, constants, onSave, onCancel }) => {
...
// Add Modal Component
const AddModal: React.FC<AddFormProps & { onClose: () => void }> = ({ type, constants, onSubmit, onClose }) => {
...
const FormFields: React.FC<FormFieldsProps> = ({ type, form, onChange, constants }) => {
