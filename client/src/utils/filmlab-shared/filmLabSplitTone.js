 ...
 * 计算分区权重 (带平衡调整)
 */
export function calculateZoneWeights(luminance, balance = 0) {
  const balanceOffset = balance / 200;
 ...
 * 检查分离色调参数是否为默认值
 */
export function isDefaultSplitTone(params) {
 ...
 * 应用分离色调到单个像素
 */
export function applySplitTone(r, g, b, params = {}) {
 ...
 * 批量应用分离色调到像素数组
 */
export function applySplitToneToArray(data, params, options = {}) {
 ...
 * 合并分离色调参数
 */
export function mergeSplitToneParams(base, overlay) {
  return {
    highlights: {
 ...
 * 验证分离色调参数
 */
export function validateSplitToneParams(params) {
  const errors = [];
...
/** 分离色调预设 */
export const SPLIT_TONE_PRESETS = {
  warmCoolFilm: {
    name: '暖高光冷阴影',
