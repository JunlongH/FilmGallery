...
import { DEFAULT_CURVES, DEFAULT_WB_PARAMS, DEBUG, FILM_PROFILES } from './filmLabConstants.js';
import { buildToneLUT } from './filmLabToneLUT.js';
import { buildCurveLUT } from './filmLabCurves.js';
import { computeWBGains } from './filmLabWhiteBalance.js';
import { applyInversion } from './filmLabInversion.js';
import { applyFilmCurve } from './filmLabCurve.js';
import { applyHSL, DEFAULT_HSL_PARAMS, isDefaultHSL } from './filmLabHSL.js';
import { applySplitTone, DEFAULT_SPLIT_TONE_PARAMS, isDefaultSplitTone } from './filmLabSplitTone.js';
 ...
 */
export function sampleLUT3D(r, g, b, lut, intensity = 1) {
...
export function processPixel(r, g, b, luts, params = {}) {
  ...
  if (params.inverted && params.filmCurveEnabled && params.filmCurveProfile) {
    ...
    const profile = FILM_PROFILES[profileKey];
 ...
 */
export function processPixelArray(data, params = {}, options = {}) {
 ...
 * 调试辅助：打印处理参数
 */
export function debugLogParams(params, luts) {
  if (!DEBUG) return;
