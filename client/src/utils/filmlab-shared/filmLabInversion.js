/**
 * FilmLab 胶片反转模块 (ESM 版本)
 * 
 * @module filmLabInversion
 * @description 实现线性反转和对数反转（纯数学操作）
 * 
 * 注意：胶片密度曲线（Film Curve）已分离到 filmLabCurve.js 模块
 * 本模块仅处理负片到正片的纯数学反转
 */

import { DEFAULT_INVERSION_PARAMS } from './filmLabConstants.js';
 ...
 */
export function invertLinear(value) {
  return 255 - value;
}
 ...
 * @returns {number} 反转后的值
 */
export function invertLog(value) {
  // +1 避免 log(0)
  return 255 * (1 - Math.log(value + 1) / Math.log(256));
}
 ...
 */
export function applyInversion(value, params = {}) {
  const {
 ...
 */
export function applyInversionRGB(r, g, b, params = {}) {
  return [
 ...
 */
export function buildInversionLUT(params = {}) {
